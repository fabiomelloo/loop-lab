...
<form class="card" method="POST" action="{{ route('playground.run') }}">@csrf<textarea class="editor" name="code" aria-label="Editor do playground" spellcheck="false">{{ old('code',"<?php\n\n\$nome = 'João';\necho 'Olá, ' . \$nome;") }}</textarea>@error('code')<p class="feedback bad">{{ $message }}</p>@enderror<button class="btn btn-primary" type="submit">Executar código</button></form>
