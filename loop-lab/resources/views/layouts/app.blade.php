    ...
    <aside class="sidebar"><div class="brand"><span>&lt;?php</span> na Prática</div>
        ...
        <nav aria-label="Módulos do curso"><a class="nav-link" href="{{ route('dashboard') }}">Visão geral</a>
            @foreach($modules as $module)<div class="module">{{ str_pad($module->position,2,'0',STR_PAD_LEFT) }} {{ $module->title }}</div>
                @foreach($module->lessons as $item)<a class="nav-link {{ request()->route('lesson')?->is($item) ? 'active' : '' }}" href="{{ route('lessons.show',$item) }}">{{ $item->title }}</a>@endforeach
            @endforeach
...
(() => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (reset) {
            ...
            form.querySelector('[name="code"]').value = new TextDecoder().decode(bytes);
    ...
    function prepareEditors() {
        document.querySelectorAll('textarea.editor').forEach(editor => {
            const key = `php-pratica:${location.pathname}:${editor.name}`;
            const saved = localStorage.getItem(key);
            if (saved && !editor.dataset.restored) editor.value = saved;
            editor.dataset.restored = '1';
            editor.addEventListener('input', () => localStorage.setItem(key, editor.value), {once: false});
            editor.addEventListener('keydown', event => {
                ...
                if (event.key === 'Enter') {
                    ...
                    if (indent) { event.preventDefault(); editor.setRangeText(`\n${indent}`, editor.selectionStart, editor.selectionEnd, 'end'); }
                }
            });
        });
    }
    prepareEditors();
    new MutationObserver(prepareEditors).observe(document.querySelector('.content'), {childList:true, subtree:true});
})();
</script>
</body></html>
