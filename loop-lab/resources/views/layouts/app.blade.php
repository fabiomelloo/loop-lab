...
<body><div class="ajax-loader" aria-hidden="true"></div><div class="shell">
    <aside class="sidebar"><div class="brand"><span>&lt;?php</span> na Prática</div>
        <div class="sidebar-card progress"><div class="progress-row"><span>Seu progresso</span><strong data-progress-percent>{{ $stats['percent'] }}%</strong></div><div class="bar"><span data-progress-bar style="width:{{ $stats['percent'] }}%"></span></div><p data-progress-count>{{ $stats['completed'] }} de {{ $stats['total'] }} exercícios concluídos. Continue na trilha para liberar a próxima etapa.</p></div>
        <nav aria-label="Módulos do curso">
            <section class="sidebar-section"><p class="sidebar-heading">Trilha guiada</p><p class="sidebar-note">Siga a ordem do topo para a base: primeiro a teoria, depois a prática e, por fim, a revisão.</p></section>
            @foreach($modules as $module)
                <details class="module-group" @if($loop->first || (request()->route('lesson') && request()->route('lesson')->module_id === $module->id)) open @endif>
                    <summary class="module-summary">
                        <div><strong>{{ str_pad($module->position,2,'0',STR_PAD_LEFT) }} {{ $module->title }}</strong><small>{{ $module->lessons->count() }} aulas</small></div>
                        <span class="module-badge">Etapa {{ str_pad($module->position,2,'0',STR_PAD_LEFT) }}</span>
                    </summary>
                    <div class="module-lessons">
                        @foreach($module->lessons as $item)
                            ...
                            <a class="nav-link {{ request()->route('lesson')?->is($item) ? 'active' : '' }} {{ in_array($item->id, $completedLessonIds ?? []) ? 'completed' : '' }} {{ $locked ? 'locked' : '' }}" @if(request()->route('lesson')?->is($item)) aria-current="page" @endif @if($locked) title="Complete a aula anterior para liberar esta etapa" @endif href="{{ route('lessons.show',$item) }}"><span>{{ $item->title }}</span><span class="nav-status">{{ in_array($item->id, $completedLessonIds ?? []) ? '✓' : str_pad($loop->iteration,2,'0',STR_PAD_LEFT) }}</span></a>
                        @endforeach
                    </div>
                </details>
            @endforeach
